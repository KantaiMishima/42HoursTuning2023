...
export const getUsers = async (
  ...
  const query = `SELECT u.user_id, u.user_name, u.office_id, u.user_icon_id, o.office_name, f.file_name FROM user AS u
...
export const getUserForFilter = async (
  ...
  if (!userId) {
    [userRows] = await pool.query<RowDataPacket[]>(
	  "SELECT user_id, user_name, office_id, user_icon_id FROM user u INNER JOIN (SELECT CEIL(RAND() * (SELECT MAX(rownum) FROM user)) AS rand) AS tmp ON u.rownum = tmp.rand"
    );
  } else {
    [userRows] = await pool.query<RowDataPacket[]>(
      "SELECT user_id, user_name, office_id, user_icon_id FROM user WHERE user_id = ?",
